...
from weekly_report import build_weekly_report, save_weekly_report_file
from wifi import get_current_wifi, scan_available_wifis
...
class TrayApp:
    ...
    def _show_menu(self, event):
        try:
            if not self.running:
                return
            if not self.root.winfo_exists() or not self.menu.winfo_exists():
                return
            self.menu.tk_popup(event.x_root, event.y_root)
        except tk.TclError:
            # 窗口关闭/销毁过程中可能仍收到一次右键事件；忽略即可。
            return
        finally:
            try:
